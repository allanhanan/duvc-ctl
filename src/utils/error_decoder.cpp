...
std::string decode_system_error(unsigned long error_code) {
    ...
    if (size > 0 && buffer) {
        ...
        int utf8_size = WideCharToMultiByte(CP_UTF8, 0, buffer, size, 
                                           nullptr, 0, nullptr, nullptr);
        if (utf8_size > 0) {
            result.resize(utf8_size);
            WideCharToMultiByte(CP_UTF8, 0, buffer, size, 
...
std::string decode_hresult(HRESULT hr) {
    ...
    if (error.ErrorMessage()) {
        ...
        int utf8_size = WideCharToMultiByte(CP_UTF8, 0, wide_msg, -1,
                                           nullptr, 0, nullptr, nullptr);
        if (utf8_size > 0) {
            description.resize(utf8_size - 1); // -1 for null terminator
            WideCharToMultiByte(CP_UTF8, 0, wide_msg, -1,
                               description.data(), utf8_size, nullptr, nullptr);
        }
    }
