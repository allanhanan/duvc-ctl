  ...
  try {
    ...
    if (!prop_set.is_valid()) {
      return Err<std::vector<uint8_t>>(
          ...
          "Device does not support vendor properties");
    }

    return prop_set.get_property(LOGITECH_PROPERTY_SET,
                                 static_cast<uint32_t>(prop));

  } catch (const std::exception &e) {
    DUVC_LOG_ERROR("Exception getting Logitech property: " +
                   std::string(e.what()));
...
Result<void> set_logitech_property(const Device &device, LogitechProperty prop,
  ...
  try {
    ...
    return prop_set.set_property(LOGITECH_PROPERTY_SET,
                                 static_cast<uint32_t>(prop), data);

  } catch (const std::exception &e) {
    DUVC_LOG_ERROR("Exception setting Logitech property: " +
                   std::string(e.what()));
...
Result<bool> supports_logitech_properties(const Device &device) {
  try {
    ...
    auto result = prop_set.query_support(
        LOGITECH_PROPERTY_SET,
        static_cast<uint32_t>(LogitechProperty::RightLight));

    if (result.is_ok()) {
      uint32_t support_flags = result.value();
      return Ok((support_flags &
                 (KSPROPERTY_SUPPORT_GET | KSPROPERTY_SUPPORT_SET)) != 0);
    }

    return Ok(false);

  } catch (const std::exception &e) {
    DUVC_LOG_DEBUG("Exception checking Logitech support: " +
                   std::string(e.what()));
...
get_logitech_property_typed<uint32_t>(const Device &, LogitechProperty);
template Result<int32_t> get_logitech_property_typed<int32_t>(const Device &,
                                                              LogitechProperty);
template Result<bool> get_logitech_property_typed<bool>(const Device &,
                                                        LogitechProperty);

template Result<void> set_logitech_property_typed<uint32_t>(const Device &,
                                                            LogitechProperty,
                                                            const uint32_t &);
template Result<void> set_logitech_property_typed<int32_t>(const Device &,
                                                           LogitechProperty,
                                                           const int32_t &);
template Result<void> set_logitech_property_typed<bool>(const Device &,
                                                        LogitechProperty,
                                                        const bool &);
