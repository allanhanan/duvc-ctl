...
long DirectShowMapper::map_camera_property(CamProp prop) {
  switch (prop) {
  case CamProp::Pan:
    return 0L; // CameraControl_Pan
  case CamProp::Tilt:
    return 1L; // CameraControl_Tilt
  case CamProp::Roll:
    return 2L; // CameraControl_Roll
  case CamProp::Zoom:
    return 3L; // CameraControl_Zoom
  case CamProp::Exposure:
    return 4L; // CameraControl_Exposure
  case CamProp::Iris:
    return 5L; // CameraControl_Iris
  case CamProp::Focus:
    return 6L; // CameraControl_Focus
  case CamProp::ScanMode:
    return 7L; // CameraControl_ScanMode
  case CamProp::Privacy:
    return 8L; // CameraControl_Privacy
  case CamProp::PanRelative:
    return 9L; // CameraControl_PanRelative
  case CamProp::TiltRelative:
    return 10L; // CameraControl_TiltRelative
  case CamProp::RollRelative:
    return 11L; // CameraControl_RollRelative
  case CamProp::ZoomRelative:
    return 12L; // CameraControl_ZoomRelative
  case CamProp::ExposureRelative:
    return 13L; // CameraControl_ExposureRelative
  case CamProp::IrisRelative:
    return 14L; // CameraControl_IrisRelative
  case CamProp::FocusRelative:
    return 15L; // CameraControl_FocusRelative
  case CamProp::PanTilt:
    return 16L; // CameraControl_PanTilt
  case CamProp::PanTiltRelative:
    return 17L; // CameraControl_PanTiltRelative
  case CamProp::FocusSimple:
    return 18L; // CameraControl_FocusSimple
  case CamProp::DigitalZoom:
    return 19L; // CameraControl_DigitalZoom
  case CamProp::DigitalZoomRelative:
    return 20L; // CameraControl_DigitalZoomRelative
  case CamProp::BacklightCompensation:
    return 21L; // CameraControl_BacklightCompensation
  case CamProp::Lamp:
    return 22L; // CameraControl_Lamp
  default:
    return -1;
  }
}
...
long DirectShowMapper::map_video_property(VidProp prop) {
  ...
  case VidProp::Brightness:
    return 0L; // VideoProcAmp_Brightness
  case VidProp::Contrast:
    return 1L; // VideoProcAmp_Contrast
  case VidProp::Hue:
    return 2L; // VideoProcAmp_Hue
  case VidProp::Saturation:
    return 3L; // VideoProcAmp_Saturation
  case VidProp::Sharpness:
    return 4L; // VideoProcAmp_Sharpness
  case VidProp::Gamma:
    return 5L; // VideoProcAmp_Gamma
  case VidProp::ColorEnable:
    return 6L; // VideoProcAmp_ColorEnable
  case VidProp::WhiteBalance:
    return 7L; // VideoProcAmp_WhiteBalance
...
long DirectShowMapper::map_camera_mode_to_flags(CamMode mode,
...
Device DirectShowEnumerator::read_device_info(IMoniker *moniker) {
  ...
  if (device.path.empty()) {
    ...
    if (SUCCEEDED(moniker->GetDisplayName(nullptr, nullptr, &display_name)) &&
        display_name) {
...
bool DirectShowFilter::is_valid() const { return static_cast<bool>(filter_); }
...
com_ptr<IAMCameraControl> DirectShowFilter::get_camera_control() {
  ...
  HRESULT hr = filter_->QueryInterface(
      IID_IAMCameraControl, reinterpret_cast<void **>(camera_control.put()));
...
com_ptr<IAMVideoProcAmp> DirectShowFilter::get_video_proc_amp() {
  ...
  HRESULT hr = filter_->QueryInterface(
      IID_IAMVideoProcAmp, reinterpret_cast<void **>(video_proc_amp.put()));
...
com_ptr<IKsPropertySet> DirectShowFilter::get_property_set() {
  ...
  HRESULT hr = filter_->QueryInterface(
      IID_IKsPropertySet, reinterpret_cast<void **>(property_set.put()));
...
public:
  explicit DirectShowDeviceConnection(const Device &device) : filter_(device) {}

  bool is_valid() const override { return filter_.is_valid(); }
  ...
  Result<PropSetting> get_camera_property(CamProp prop) override {
    ...
    if (FAILED(hr)) {
      return Err<PropSetting>(ErrorCode::SystemError,
  ...
  Result<void> set_camera_property(CamProp prop,
    ...
    HRESULT hr =
        cam_ctrl->Set(prop_id, static_cast<long>(setting.value), flags);
  ...
  Result<PropRange> get_camera_property_range(CamProp prop) override {
    ...
    HRESULT hr =
        cam_ctrl->GetRange(prop_id, &min, &max, &step, &default_val, &flags);

    if (FAILED(hr)) {
      return Err<PropRange>(ErrorCode::SystemError,
    ...
    range.default_mode =
        DirectShowMapper::map_flags_to_camera_mode(flags, true);
  ...
  Result<void> set_video_property(VidProp prop,
    ...
    long flags =
        DirectShowMapper::map_camera_mode_to_flags(setting.mode, false);
    HRESULT hr =
        vid_proc->Set(prop_id, static_cast<long>(setting.value), flags);
  ...
  Result<PropRange> get_video_property_range(VidProp prop) override {
    ...
    long min = 0, max = 0, step = 0, default_val = 0, flags = 0;
    HRESULT hr =
        vid_proc->GetRange(prop_id, &min, &max, &step, &default_val, &flags);
    ...
    range.default_val = static_cast<int>(default_val);
    range.default_mode =
        DirectShowMapper::map_flags_to_camera_mode(flags, false);
